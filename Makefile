...
# Add file system information structure (already comes with the stage1 binary)
# dd if=$(BOOT_BIN)/FSInfo_FAT32.bin 	of=$(FAT_32) 	bs=512 	seek=1	conv=notrunc
# Add copy of bootsector (already comes with the stage1 binary)
	dd if=$(BOOT_BIN)/boot_FAT32.bin		of=$(FAT_32) 	bs=512 	seek=6	conv=notrunc
# Add copy of system information structure
# dd if=$(BOOT_BIN)/FSInfo_FAT32.bin 	of=$(FAT_32) 	bs=512 	seek=7	conv=notrunc
# Add stage 2 bootloader
