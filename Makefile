...
bootloader: boot_dirs

# Rule to iterate through each subdirectory and call its Makefile
boot_dirs:
	$(foreach subdir, $(shell find $(SRC_DIR)/bootloader -type f -name Makefile -exec dirname {} \;), $(MAKE) -C $(subdir);)

kernel: kernel_dirs

kernel_dirs:
	$(foreach subdir, $(shell find $(SRC_DIR)/kernel -type f -name Makefile -exec dirname {} \;), $(MAKE) -C $(subdir);)
...
# Add stage 2 bootloader
	dd if=$(BIN)/boot2.bin 			of=$(FAT_16) 	bs=512 	seek=1	conv=notrunc
# Add files to the OS
