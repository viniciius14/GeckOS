...
void fdc_reset(void) {
    outb(FDC_DIGITAL_OUT, 0x00);    /* Disable the FDC */

    for(uint8_t i = 0 ; i != 50 ; i++) {
        io_wait();                      /* Wait */
    }

    outb(FDC_DIGITAL_OUT, 0x0C);    /* Enable the FDC */
